...
import { checkDormancyBreak, checkFundingScale, checkTransactionAnomaly } from "./ruleDiff.js";
...
export async function runPipeline(
  ...
  for (const signal of incomingSignals) {
    // ── Numeric / transaction signals — pure rules, no LLM ──
    if (signal.sourceType === "transaction") {
      const s = checkTransactionAnomaly(baseline, recentTxs) ?? checkDormancyBreak(baseline, recentTxs);
      if (s) {
        scores.push(s);
        stageTrace.push(`Rule diff: ${signal.signalId} → ${s.category} (magnitude ${s.magnitude}).`);
      } else {
        stageTrace.push(`Rule diff: ${signal.signalId} → within normal range, discarded (no LLM cost).`);
      }
      continue;
    }
    if (signal.sourceType === "funding_db") {
      ...
      if (s) {
        scores.push(s);
        stageTrace.push(`Rule diff: ${signal.signalId} → funding scale change (magnitude ${s.magnitude}).`);
      }
      continue;
    }

    // ── Narrative signals — embedding gate BEFORE any LLM call ──
    const embedScores = await scoreNarrativeSignal(baselineEmbedding, archetypeEmbeddings, signal.rawText ?? "");
    const worthReviewing =
      embedScores.baselineSimilarity < 0.6 || (embedScores.archetypeMatches[0]?.similarity ?? 0) > 0.55;
    ...
    const evidence = await fetchEvidenceViaMCP(signal);
